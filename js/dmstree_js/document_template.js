...
//                alert(validateTables()); 
                    ...
                    if(result == true)
                {
                         if(validateTables())
                    {
                        ...
                        dialogContent+='<input type="reset" class="btn" value="Reset">';
                        //dialogContent+='<input type="button" class="btn btn-danger" value="Reset" onclick="#">'; //<i class="icon-trash icon-white"></i>
                       // dialogContent+='<input type="button" class="btn btn-success" value="Submit" onclick="valid_form()">'; //<i class="icon-ok-sign icon-white"></i>
                        dialogContent+='</div>';
                        ...
                        dialogContent+= '\n</form>';
                        //alert(dialogContent);
                       /* $('#template_fromdata').val(dialogContent);
                        ...
                        //newTemplate += '<link rel="stylesheet" href="//code.jquery.com/ui/1.10.4/themes/smoothness/jquery-ui.css">';
                        newTemplate += '<link  rel="stylesheet" href="bootstrap/css/bootstrap.min.css" media="screen">';
                        //newTemplate += '<script src="bootstrapvalidator-0.5.0/vendor/jquery/jquery-1.10.2.min.js"></script>';
                        //newTemplate += '<script src="//code.jquery.com/ui/1.10.4/jquery-ui.js"></script>';
                       // newTemplate += '<script type="text/javascript" src="js/dmstree_js/moment.min.js"></script>';
                ...
                function compileTemplates() {
                    ...
                    window.templates.stdlistbox = Handlebars.compile($("#stdlistbox-template").html());
                }
                ...
                load_values.stdcombobox = function(ctrl_type, ctrl_id) {
                    ...
                    var ctrl = div_ctrl.find("select")[0];
                    form.find("[name=name]").val(ctrl.name)
                    var defaulttno = 'combo'; 
                    var combooptions = '';
                    $.ajax({
                            type:'post',
                            url:'getstandared_combolist.php?',               //get_stdlist.php defaulttno : defaulttno
                            data:{
                            ...
                            success:function(data){
                                ...
                                combooptions += '<option value= "">--Select--</option>';
                                for(var comboindex =0; comboindex < response.length; comboindex++)
                                   {
                                       var list_name = response[comboindex].ListName;
                                       combooptions += '<option value= "'+list_name+'" >'+list_name+'</option>';
                                   }
                                    form.find("#scombo").append(combooptions);
                ...
                /*Multi-select stdlistbox has same as customization feature*/
                
                load_values.stdlistbox = function(ctrl_type, ctrl_id) {
                    var form = $("#theForm");
                    var div_ctrl = $("#"+ctrl_id);
                    var ctrl = div_ctrl.find("select")[0];
                    form.find("[name=name]").val(ctrl.name)
                    ...
                    $.ajax({
                            ...
                            success:function(data){
                                var response = $.parseJSON(data);
                                listoptions += '<option value= "">--Select--</option>';
                                for(var listindex =0; listindex < response.length; listindex++)
                                   {
                                       var list_name = response[listindex].ListName;
                                       listoptions += '<option value= "'+list_name+'" >'+list_name+'</option>';
                                   }
                ...
                /* Specific method to save changes to a combobox*/
                /* Specific method to save changes to a combobox */
                save_changes.stdlistbox = function(values) {
                    console.log(values);
                    var div_ctrl = $("#"+values.forCtrl);
                    var ctrl = div_ctrl.find("select")[0];
                    var stdboxname = values.label;
