function display_combooptions(comboname,stdtype)
{
   var defaulttno = ''; 
   if(stdtype == 'combo')
       {
           defaulttno = 'combooptions';
       }
    else
        {
           defaulttno = 'listoptions'; 
        }
   if(comboname != '')
   {
       $.ajax({
               type:'post',
               url:'getstandared_combolist.php?',      
               data:{
                       stdcntrltype : defaulttno,
                       comboname    : comboname
                    },
               success:function(data){
                   var response = $.parseJSON(data);
                   var combovalues = '';
                   for(var comboindex = 0; comboindex < response.length; comboindex++)
                   {
                       var combolist = response[comboindex].List;
                       for(var combooptindex = 0 ;combooptindex < combolist.length; combooptindex++ )
                           {
                               var isactive = combolist[combooptindex].IsActive;
                               if(isactive == true)
                                {
                                    combovalues += combolist[combooptindex].Description+'\n';
                                    if(stdtype == 'combo')
                                        {
                                            $('#std_seletedcombo').find('textarea').val(combovalues);
                                        }
                                    else
                                        {
                                            $('#std_seletedlist').find('textarea').val(combovalues);
                                        }
                                }
                           }
                   }
               }
        });
        if(stdtype == 'combo')
          $('.stdcomboname').val(comboname);
        else
          $('.stdlistname').val(comboname);  
   }    
   else
   {
      $('#std_seletedlist').find('textarea').val('');
   }    
}
                
function readURL(input) {
    
                        var formValues = {};
                        var val=null;
                        var cntrl_id = '';
                        $("#theForm").find("input, textarea, select").each(function(i,o) {
                            if(o.name=='forCtrl')
                                    {
                                        cntrl_id = o.value;
                                    }
                            if(o.name=="form_image")
                                    {
                                       
                                        if (input.files && input.files[0]) {
                                                var reader = new FileReader();

                                            reader.onload = function (e) {
                                                $('#'+cntrl_id).find('.fimage')
                                                    .attr('src', e.target.result)
                                                   /* .width(120)
                                                    .height('auto')*/;
                                            };

                                                 reader.readAsDataURL(input.files[0]);
                                        }
                                    }
                        });
        
    }
    
    
    function open_logoimage_popup()
    {
        $('#customization_logo_modal').css('display','block');
    }
    
    function readURLlogo(input1) {
       
        if (input1.files && input1.files[0]) {
            var reader = new FileReader();

            reader.onload = function (e) {
                $('#logo_image')
                    .attr('src', e.target.result)
                    .width('100%')
                    .height('auto');
            };

            reader.readAsDataURL(input1.files[0]);
        }
    }
    
    function save_logochanges()
    {
//        save_changes.common();
         $('#headerForm').val('');   
         $('#customization_logo_modal').css('display','none');
        
    }
    
    function cancel_formlogo()
    {
        $('#customization_logo_modal').css('display','none');
    }
    
    function open_title_popup()
    {
       $('#customization_title_modal').css('display','block'); 
    }
    function save_titlechanges()
    {
        var formtitle = $('#handlebars-title-label').val();
        $('#form-title').val(formtitle);
        var titlefont =$('#form-title').closest('div').find('.titlelabel_font').val();
        $('#form-title').addClass(titlefont);
        var titlesize =$('#form-title').closest('div').find('.titlelabel_size').val();
//        alert(($('#form-title').attr('class')).split(" "));
        $('#form-title').addClass(titlesize);
        $('#customization_title_modal').css('display','none'); 
    }
    function cancel_formtitle()
    {
        $('#customization_title_modal').css('display','none'); 
    }
    function change_titlefont(titlefont)
    {
        if(titlefont == 'bold')
            {
                $('#form-title').removeClass('italicfont');
                $('#handlebars-title-label').css("font-style","normal");
                $('#handlebars-title-label').css("font-weight","bold");
                $('#form-title').closest('div').find('.titlelabel_font').val('boldfont');
            }
       else if(titlefont == 'italic')
            {
                $('#form-title').removeClass('boldfont');
                $('#handlebars-title-label').css("font-weight","normal");
                $('#handlebars-title-label').css("font-style","italic");
                $('#form-title').closest('div').find('.titlelabel_font').val('italicfont');
            } 
       else if(titlefont == 'normal')
            {
                $('#form-title').removeClass('italicfont');
                $('#form-title').removeClass('boldfont');
                $('#handlebars-title-label').css("font-weight","normal");
                $('#handlebars-title-label').css("font-style","normal");
                $('#form-title').closest('div').find('.titlelabel_font').val('');
            }     
    }
    
    function change_titlesize(titlesize)
    {
                        if(titlesize == '22')
                        {
                            $('#form-title').removeClass('midtitle');
                            $('#form-title').removeClass('largetitle');
                            $('#handlebars-title-label').css("font-size","22px");
                            $('#form-title').closest('div').find('.titlelabel_size').val('smalltitle');
                        } 
                        else if(titlesize == '24')
                        {
                            $('#form-title').removeClass('smalltitle');
                            $('#form-title').removeClass('largetitle');
                            $('#handlebars-title-label').css("font-size","24px");
                            $('#form-title').closest('div').find('.titlelabel_size').val('midtitle');    
                        }
                        else if(titlesize == '26')
                        {
                            $('#form-title').removeClass('smalltitle');
                            $('#form-title').removeClass('midtitle');
                            $('#handlebars-title-label').css("font-size","26px");
                            $('#form-title').closest('div').find('.titlelabel_size').val('largetitle');
                        }   
    }
    
    
    function display_minmaxlen_err(lenobj)
        {
            
                var flag1 =0;
                var inputval = $(lenobj).val();
                var parentid = $(lenobj).closest("div").parent(".droppedField").attr("id");
                var minlen = $(lenobj).closest("div").find(".min_length").val();
                var maxlen = $(lenobj).closest("div").find(".max_length").val();
                if((minlen == '' && maxlen == '') || (minlen == '0' && maxlen =='0'))
                    {
                        flag1 = 1;
                    }
                else
                    {
                        if( minlen == '0')
                            {
                                minlen = '';
                            }
                        if( maxlen == '0')
                            {
                                maxlen == '';
                            }
                        var inputlen = inputval.length;
                        if( minlen != '' && maxlen != '' && parseInt(inputlen) < parseInt(minlen)  && parseInt(minlen) < parseInt(maxlen))
                            {
                            $('#'+parentid).find('.required_msg').text('This field required min '+minlen+' and max '+maxlen+' characters');
                            }
                        else if(minlen != '' && parseInt(inputlen) < parseInt(minlen)  && maxlen == '')
                            {
                                $('#'+parentid).find('.required_msg').text('This field required min '+minlen+' characters');
                            }
                        else if(minlen == '' && parseInt(inputlen) < parseInt(maxlen) && maxlen != '')
                            {
                                $('#'+parentid).find('.required_msg').text('This field required max '+maxlen+' characters');
                            }
                            else
                                {
                                $('#'+parentid).find('.required_msg').text(''); 
                                }
                    }
        }
    
    function create_table()
    {
                $('#table_modelbox').html('');
                var cntrl_id = '';
                var ctrl_type = '';
                var row_opt = '';
                var col_opt = '';
                $("#theForm").find("input, textarea, select").each(function(i,o) {
                    //alert(o.name+'--'+o.value);
                    if(o.name=='type')
                        {
                            ctrl_type = o.value; 
                        }
                    if(o.name=='forCtrl')
                    {
                        cntrl_id = o.value;
                    }
                    if(o.name == 'tbl_row_header')
                    {
                        row_opt = o.checked;	
                    }
                    if(o.name == 'tbl_col_header')
                    {
                        col_opt = o.checked;
                    }
                });  
                if(ctrl_type == 'table')
                {
                    $('#'+cntrl_id).find('table').attr('id','tbl-'+cntrl_id);
                    var tab_rows = $('#'+cntrl_id).find('.table_rows').val();
                    var tab_cols = $('#'+cntrl_id).find('.table_cols').val();

                    if(tab_rows == '' && tab_cols == '')
                        {
                            $('#form_table').html('');
                            $('#table_modelbox').css('display','none');
                        }
                    else
                        {
                            var tbldialog = '';
                            if(cntrl_id != '' && tab_cols != '' && tab_rows != '' && row_opt == true && col_opt == true)
                            {
                                 tbldialog = colrowname_popupbox(cntrl_id,tab_cols,tab_rows);
                            }
                            else if(cntrl_id != '' && tab_cols != '' && tab_rows != '' && row_opt == false && col_opt == true)
                            {
                                 tbldialog = colname_popupbox(cntrl_id,tab_cols,tab_rows);
                            }   
                            else if(cntrl_id != '' && tab_cols != '' && tab_rows != '' && row_opt == true && col_opt == false)
                            {
                                 tbldialog = rowname_popupbox(cntrl_id,tab_cols,tab_rows);
                            }
                            else if(cntrl_id != '' && tab_cols != '' && tab_rows != '' && row_opt == false && col_opt == false)
                            {
                                 tbldialog = colname_popupbox(cntrl_id,tab_cols,tab_rows);
                            } 
                            else if(cntrl_id != '' && tab_cols != '' && tab_rows == '')
                            {
                                 tbldialog = changeCols_popupbox(cntrl_id,tab_cols);     
                            }
                            
//                            if(cntrl_id != '' && tab_cols != '' && tab_rows != '')
//                            {
//                                 tbldialog = colname_popupbox(cntrl_id,tab_cols,tab_rows);
//                            }    
//                            else if(cntrl_id != '' && tab_cols != '' && tab_rows == '')
//                            {
//                                 tbldialog = changeCols_popupbox(cntrl_id,tab_cols);     
//                            }
//                            alert($('#'+cntrl_id).find('#form_table tbody').html());
//                            $('#'+cntrl_id).find('#form_table tbody').html('');
                            $('#table_modelbox').html('');
//                            alert($('#'+cntrl_id).find('#form_table tbody').html());
                            $('#table_modelbox').append(tbldialog);
                            $('#table_modelbox').css('display','block');
                        }
                    
                }
      
    }
    
    
    function colrowname_popupbox(tab_id,tab_cols,tab_rows)
    {
        var tblpopup = '';
        var cols = tab_cols;
		var rows = tab_rows;
        var tablediv_id = tab_id;
        for(var i = 1; i <= cols; i++)
            {
               tblpopup +='<p class="row"><label class="col-md-3">Column'+i+' Label: </label><input class="col-md-9 col" type="text" id="col'+i+'" value=""></p>';
            }
		for(var j = 1; j <= rows; j++)
			{
					tblpopup +='<p class="rows"><label class="col-md-3">Row'+j+' Label: </label><input class="col-md-9 row-record" type="text" id="rowrecord'+j+'" value=""></p>';
			}
        var tableDialog  = '';
        tableDialog += '<div id="customization_table_modal" name="customization_table_modal" class="modal hide fade in titlepopup" aria-hidden="false" style="display: block;">';
        tableDialog += '<div class="modal-header"><h3>Table Header</h3></div>';
        tableDialog += '<div class="modal-body"><form id="table_header" class="form-horizontal">';
        tableDialog += tblpopup;
        tableDialog += '</div>';
        tableDialog += '<div class="modal-footer">';
        tableDialog += '<button class="btn btn-primary" onclick="get_columnrowHeader('+cols+','+tab_rows+',\''+tablediv_id+'\')">Save Changes</button>';
        tableDialog += '<button class="btn" onclick="cancel_tablechanges()">Cancel</button>';
        tableDialog += '</div></div>';
        return tableDialog;
    }
    
    
    function colname_popupbox(tab_id,tab_cols,tab_rows)
    {
        var tblpopup = '';
        var cols = tab_cols;
        var tablediv_id = tab_id;
        for(var i = 1; i <= cols; i++)
            {
               tblpopup +='<p class="row"><label class="col-md-3">Column'+i+' Label: </label><input class="col-md-9 col" type="text" id="col'+i+'" value=""></p>';
            }
        var tableDialog  = '';
        tableDialog += '<div id="customization_table_modal" name="customization_table_modal" class="modal hide fade in titlepopup" aria-hidden="false" style="display: block;">';
        tableDialog += '<div class="modal-header"><h3>Table Header</h3></div>';
        tableDialog += '<div class="modal-body"><form id="table_header" class="form-horizontal">';
        tableDialog += tblpopup;
        tableDialog += '</div>';
        tableDialog += '<div class="modal-footer">';
        tableDialog += '<button class="btn btn-primary" onclick="get_columnHeader('+cols+','+tab_rows+',\''+tablediv_id+'\')">Save Changes</button>';
        tableDialog += '<button class="btn" onclick="cancel_tablechanges()">Cancel</button>';
        tableDialog += '</div></div>';
        return tableDialog;
    }
    /*function colname_popupbox(tab_id,tab_cols,tab_rows)
    {
        var tblpopup = '';
        var cols = tab_cols;
        var tablediv_id = tab_id;
        for(var i = 1; i <= cols; i++)
            {
               tblpopup +='<p class="row"><label class="col-md-3">Column'+i+' Label: </label><input class="col-md-9 col" type="text" id="col'+i+'" value=""></p>';
            }
        var tableDialog  = '';
        tableDialog += '<div id="customization_table_modal" name="customization_table_modal" class="modal hide fade in titlepopup" aria-hidden="false" style="display: block;">';
        tableDialog += '<div class="modal-header"><h3>Table Header</h3></div>';
        tableDialog += '<div class="modal-body"><form id="table_header" class="form-horizontal">';
        tableDialog += tblpopup;
        tableDialog += '</div>';
        tableDialog += '<div class="modal-footer">';
        tableDialog += '<button class="btn btn-primary" onclick="get_columnHeader('+cols+','+tab_rows+',\''+tablediv_id+'\')">Save Changes</button>';
        tableDialog += '<button class="btn" onclick="cancel_tablechanges()">Cancel</button>';
        tableDialog += '</div></div>';
        return tableDialog;
    }*/
    
    
    
    
    function rowname_popupbox(tab_id,tab_cols,tab_rows)
	{
        var tblpopup = '';
		var rows = tab_rows;
        var tablediv_id = tab_id;
		for(var j = 1; j <= rows; j++)
			{
					tblpopup +='<p class="rows"><label class="col-md-3">Row'+j+' Label: </label><input class="col-md-9 row-record" type="text" id="rowrecord'+j+'" value=""></p>';
			}
        var tableDialog  = '';
        tableDialog += '<div id="customization_table_modal" name="customization_table_modal" class="modal hide fade in titlepopup" aria-hidden="false" style="display: block;">';
        tableDialog += '<div class="modal-header"><h3>Table Header</h3></div>';
        tableDialog += '<div class="modal-body"><form id="table_header" class="form-horizontal">';
        tableDialog += tblpopup;
        tableDialog += '</div>';
        tableDialog += '<div class="modal-footer">';
        tableDialog += '<button class="btn btn-primary" onclick="get_rowHeader('+tab_cols+','+tab_rows+',\''+tablediv_id+'\')">Save Changes</button>';
        tableDialog += '<button class="btn" onclick="cancel_tablechanges()">Cancel</button>';
        tableDialog += '</div></div>';
        return tableDialog;
    
	}
        
        
    function validate_tblModalBody()
    {
        var inputflag = 0;
        $('#table_header p').each(function(){
            var inputval = $(this).find('input').val();
            //alert(inputval);
            if(inputval == '')
                {
                    inputflag = 1;
                }
        });
        if(inputflag == 1)
            {
                return false;
            }
        else
            {
                return true;
            }
    }    
    function get_columnrowHeader(cols,rows,tblid)
	{
            
          if(validate_tblModalBody())
              {
                $('#'+tblid).find('.tbl_rows').val(rows);
                $('#'+tblid).find('.tbl_cols').val(cols);  
		var colTitle = new Array();
        $('#table_header p.row').each(function(){
            var colum_heading = $(this).find('input.col').val();
            colTitle.push(colum_heading);
        });
		
		var rowTitle = new Array();
        $('#table_header p.rows').each(function(){
            var row_heading = $(this).find('input.row-record').val();
            rowTitle.push(row_heading);
        });

        var no_cols = parseInt(cols)+ parseInt(1);
        var ctrl_id = tblid;
        var no_rows = parseInt(rows)+ parseInt(1);
        
        var tblContent = '';
        var count = 1;
        for(var tblrow = 0; tblrow < no_rows; tblrow++)
            {
                tblContent += '<tr id="record'+count+'">';
                for(var tblcols = 0; tblcols < no_cols; tblcols++)
                    {
                        if(tblrow == 0 && tblcols == 0) 
                           {
								tblContent += '<td><input type="text" class="tbl_td" readonly></td>';
                               
                           }
						else if(tblrow == 0 && tblcols != 0)
							{
								tblContent += '<th>'+colTitle[tblcols - 1]+'</th>';	
						    }				
                        else if(tblrow != 0 && tblcols == 0)
							{
								tblContent += '<th>'+rowTitle[tblrow-1]+'</th>';
							}
						else		
                           {
                               tblContent += '<td><input type="text" class="tbl_td"></td>';
                           }
                    }
                tblContent += '</tr>'
                count++;
            }
        $('#'+ctrl_id).find('table tbody').html('');    
        $('#'+ctrl_id).find('table').append(tblContent);    
        $('#table_modelbox').css('display','none');
              }
              else
                {
                     alert("Table's Row-Column Heading can't be empty");
                }
	}
    
    function get_columnHeader(cols,rows,tblid)
    {
        if(validate_tblModalBody())
            {
            $('#'+tblid).find('.tbl_rows').val(rows);
            $('#'+tblid).find('.tbl_cols').val(cols);    
        var colTitle = new Array();
        $('#table_header p').each(function(){
            var colum_heading = $(this).find('input.col').val();
            colTitle.push(colum_heading);
        });
        var no_cols = parseInt(cols);
        var ctrl_id = tblid;
        var no_rows = parseInt(rows)+ parseInt(1);
        
        var tblContent = '';
        var count = 1;
        for(var tblrow = 0; tblrow < no_rows; tblrow++)
            {
                tblContent += '<tr id="record'+count+'">';
                for(var tblcols = 0; tblcols < no_cols; tblcols++)
                    {
                       if(tblrow == 0) 
                           {
                               tblContent += '<th>'+colTitle[tblcols]+'</th>';
                           }
                       else
                           {
                               tblContent += '<td><input type="text" class="tbl_td"></td>';
                           }
                    }
                tblContent += '</tr>'
                count++;
            }
				$('#'+ctrl_id).find('.addstatus').val("cols");
        $('#'+ctrl_id).find('table tbody').html('');    
        $('#'+ctrl_id).find('table').append(tblContent);    
        $('#table_modelbox').css('display','none');
            }
            else
                {
                     alert("Table's Column Heading can't be empty");
                }
		
    }
	
	function get_rowHeader(cols,rows,tblid)
    {
        if(validate_tblModalBody())
            {
                
            $('#'+tblid).find('.tbl_rows').val(rows);
            $('#'+tblid).find('.tbl_cols').val(cols);
        var rowTitle = new Array();
        $('#table_header p').each(function(){
            var colum_heading = $(this).find('input.row-record').val();
            rowTitle.push(colum_heading);
        });
        var no_cols = parseInt(cols);
        var ctrl_id = tblid;
        var no_rows = parseInt(rows); //+ parseInt(1)
        
        var tblContent = '';
        var count = 1;
        for(var tblrow = 0; tblrow < no_rows; tblrow++)
            {
                tblContent += '<tr id="record'+count+'">';
                for(var tblcols = 0; tblcols < no_cols; tblcols++)
                    {
                       if(tblcols == 0) 
                           {
                               tblContent += '<th>'+rowTitle[tblrow]+'</th>';
                           }
                       else
                           {
                               tblContent += '<td><input type="text" class="tbl_td"></td>';
                           }
                    }
                tblContent += '</tr>'
                count++;
            }
        $('#'+ctrl_id).find('table tbody').html('');    
        $('#'+ctrl_id).find('table').append(tblContent);    
        $('#table_modelbox').css('display','none');
        }
        else
            {
                alert("Table's Row Heading can't be empty");
            }
    }
    
    
    
   /* function get_columnHeader(cols,rows,tblid)
    {
        var colTitle = new Array();
        $('#table_header p').each(function(){
            var colum_heading = $(this).find('input.col').val();
            colTitle.push(colum_heading);
        });
//        alert(colTitle.join("\n"));
        var no_cols = parseInt(cols);
        var ctrl_id = tblid;
        var no_rows = parseInt(rows)+ parseInt(1);
        
        var tblContent = '';
        var count = 1;
        for(var tblrow = 0; tblrow < no_rows; tblrow++)
            {
                tblContent += '<tr id="record'+count+'">';
                for(var tblcols = 0; tblcols < no_cols; tblcols++)
                    {
//                        alert(colTitle[tblcols]);
                       if(tblrow == 0) 
                           {
                               tblContent += '<th>'+colTitle[tblcols]+'</th>';
                           }
                       else
                           {
                               tblContent += '<td><input type="text" class="tbl_td"></td>';
                           }
                    }
                tblContent += '</tr>'
                count++;
            }
        $('#'+ctrl_id).find('table tbody').html('');    
        $('#'+ctrl_id).find('table').append(tblContent);    
        $('#table_modelbox').css('display','none');

    }*/
    
    
    function cancel_tablechanges()
    {
        $('#table_modelbox').css('display','none');
    }
    
    
    function add_rowToTable(add_btncntrl)
    {
        var colcount = 0;
        var cntrl_id = $(add_btncntrl).closest('div').parent().parent().attr('id');
        var no_cols = $('#'+cntrl_id).children().find('.table_div table tbody tr:first th').length;
        var trContent = '';
        var record_id = parseInt(no_cols) + 1;
        trContent += '<tr id="record'+record_id+'">';
        for(var i = 0; i < no_cols; i++)
            {
               trContent +='<td><input type="text" class="tbl_td"></td>';
            }
        trContent +='</tr>';  
        $('#'+cntrl_id).children().find('.table_div table tbody').append(trContent);
//        alert($('#'+cntrl_id).children().find('.table_div table tbody').html());
    }
    
    function valid_form()
    {
        var textflag = 0;
        var passwflag = 0;
        var textareaflag = 0;
        var selectflag = 0;
        var radioflag = 0;
        var chkflag = 0;
        
            $('#doc_template .required_field').each(function(){
                if($(this).is('input'))
                    {
                        var cntrl_id = $(this).parent().parent().attr('id');
//                        alert(cntrl_id);
                        var inputType = $(this).attr('type');
                        if(inputType == 'text')
                            {
                                if($(this).val() == '')
                                    {
                                        textflag = 1;
                                        $(this).parent().find('.required_msg').text('This is required Text field');
                                    }
                                else
                                    {
                                        $(this).parent().find('.required_msg').text('');
                                    }
                            }
                        else if(inputType == 'password')
                            {
                                if($(this).val() == '')
                                {
                                    passwflag = 1;
                                    $(this).parent().find('.required_msg').text('This is required Password field');
                                }  
                                else
                                    {
                                        $(this).parent().find('.required_msg').text('');
                                    }
                            }
                      
                    }
                    if($(this).is('textarea'))    
                    {
                        var cntrl_id = $(this).parent().parent().attr('id');
//                        alert("textarea id:"+cntrl_id);
                        if($(this).val() == '')
                                {
                                    textareaflag = 1;
                                    $(this).parent().find('.required_msg').text('This is required Textarea');
                                } 
                                else
                                    {
                                        $(this).parent().find('.required_msg').text('');
                                    }
                    }
                 if($(this).is('select'))
                    {
                        var cntrl_id = $(this).parent().parent().attr('id');
                        if( ($(this).val() == '') || ($(this).val() == null))
                        {
                            selectflag = 1;
                            $(this).parent().find('.required_msg').text('This is required Select List');
                        }
                        else
                            {
                                $(this).parent().find('.required_msg').text('');
                            }
                    }
                 if($(this).is('div'))
                    {
                        var cntrl_id = $(this).parent().attr('id');
//                        alert("div id:"+cntrl_id);
                        var radioValFlag = 0;
                        var chkValFlag = 0;
                        
                        $(this).children().each(function(){
                            
                            if($(this).children().attr('type') == 'radio')
                                {
                                   if($(this).children().is(':checked'))
                                    {
                                        radioValFlag = 1;
                                    } 
                                }
                            else if($(this).children().attr('type') == 'checkbox')
                                {
                                    if($(this).children().is(':checked'))
                                    {
                                        chkValFlag = 1;
                                    } 
                                }
                            
                        });
                       
                        if($(this).children().children().attr('type') == 'radio')
                            {
                                if( radioValFlag != '1')
                                {
                                    radioflag = 1;
                                    $(this).parent().find('.required_msg').text('This is Required RadioField');
                                }
                                else
                                    {
                                        $(this).parent().find('.required_msg').text('');
                                    }
                            }
                        else if($(this).children().children().attr('type') == 'checkbox')
                            {
                                if(chkValFlag != '1')
                                {
                                    chkflag = 1;
                                    $(this).parent().find('.required_msg').text('This is Required CheckboxField');
                                }
                                else
                                    {
                                        $(this).parent().find('.required_msg').text('');
                                    }
                            }
                    }
                 
            });
            
            if(textflag == 0 && passwflag == 0 && textareaflag == 0 && selectflag == 0 && radioflag == 0 && chkflag == 0)
                {
                    return true;
                }
            else
                {
                    return false;
                }
    }
    
    
   
   
   
   function set_imageSize()
   {
       var ctrl_type = '';
       var cntrl_id = '';
       var img_size = '';
       $("#theForm").find("input, textarea, select").each(function(i,o) {
                   // alert(o.name+'--'+o.value);
                    if(o.name=='type')
                        {
                            ctrl_type = o.value; 
                        }
                    if(o.name=='forCtrl')
                    {
                        cntrl_id = o.value;
                    }
                    if(o.name=='img_ctrl')
                        {
                            img_size =$('input:radio[name=img_ctrl]').filter(":checked").val();
                          // alert("img"+img_size);
                        }
       });
       if(img_size == 'small')
           {
               $('#'+cntrl_id).find('img').attr('width','30%');
               $('#'+cntrl_id).find('img').attr('height','auto');
           }
       else if(img_size == 'medium')   
           {
               $('#'+cntrl_id).find('img').attr('width','60%');
               $('#'+cntrl_id).find('img').attr('height','auto');
           }
       else if(img_size == 'large')   
           {
               $('#'+cntrl_id).find('img').attr('width','100%');
               $('#'+cntrl_id).find('img').attr('height','auto');
           } 
       else 
           {
//               alert("in else part");
                $('#'+cntrl_id).find('img').attr('width','100%');
                $('#'+cntrl_id).find('img').attr('height','auto');
           }
   }
   
   
   function validateTables()
   {
       var flag = 0;
       $('#selected-content .table_div').each(function(){
           var rows = $(this).find('.tbl_rows').val();
           //alert(rows);
           var cols = $(this).find('.tbl_cols').val();
//           alert(cols);
           //var label = $(this).parent().attr();
//           alert("rows : "+rows+" and cols : "+cols);
           if((rows == '' && cols == '') || (rows != '' && cols == '') ||(rows == '' && cols !=''))
               {
                   alert("Tables can't be empty");
                   flag = 1;
                   return false;
               }
       });
       if(flag == 1)
           {
               return false;
           }
       else{ 
           return true; 
       }    
   }
   
   


function validateTemplateForm()
{
    var legend_text = $("#form-title")[0].value;
    if(legend_text != '')
        {
            
            var selected_content =  $("#selected-content").clone();
            var controlLength    =  selected_content.find('.droppedField').length;
            /*var selected_content_html = selected_content.html().find('.droppedField');
            return controlLength;*/
            if(controlLength == '0')
                {
                    return false;
                }
            else
                {
                    var ctrlName = new Array();
                    var ctrlIndex = 0 ;
                    var ctrlnameIndex ='' ;
                    var flag = 0;
                    var msg = '';
                    $("#selected-content").clone().find('.droppedField').each(function(){
                        if($(this).find('.cntrol_comp').length)
                            {
                                var controlName = $(this).find('.cntrol_comp').attr('name');
                                if(controlName != undefined)
                                    {
                                        ctrlnameIndex = ctrlName.indexOf(controlName);
                                        if(ctrlnameIndex == -1)
                                        {
                                            ctrlName[ctrlIndex] = controlName;
                                            ctrlIndex++;
                                        }    
                                        else
                                        {
                                            flag = 1;
                                            msg = controlName;
                                            return false;
                                        }
                                    }
                                else 
                                    {
                                        var subctrlName = $(this).find('.cntrol_comp').children().children().attr('name');
                                        ctrlnameIndex = ctrlName.indexOf(subctrlName);
                                        if(ctrlnameIndex == -1)
                                        {
                                        ctrlName[ctrlIndex] = subctrlName;
                                        ctrlIndex++;
                                        }
                                        else
                                        {
                                           flag = 1;
                                           msg = subctrlName;
                                           return false;
                                        }
                                    }
                            }
                        
                    });
                    if(flag == 0)
                        {
                            //alert(ctrlName);
                            return true;
                        }
                    else
                        {
                            
                            return msg;
                        }
                }
        }
    else
        {
            return false;
        }
}